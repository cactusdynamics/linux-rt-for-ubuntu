...
cd /workspace
git clone -b Ubuntu-5.15.0-53.59 --depth=1 https://git.launchpad.net/~ubuntu-kernel/ubuntu/+source/linux/+git/jammy linux-src
mkdir preempt-rt-patches
cd preempt-rt-patches
wget https://mirrors.edge.kernel.org/pub/linux/kernel/projects/rt/5.15/older/patches-5.15.65-rt49.tar.gz
tar xvf patches*
cd ../linux-src
